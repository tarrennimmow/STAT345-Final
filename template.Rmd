...
reset_board <- function() {
  ...
  board_squares <- tibble("Tile Name" = tiles, Class = NA, Occupants = NA, Owner = NA,"Mortgage Price" = c(0, 30, 0, 30, 0, 100, 50, 0, 50, 60,0, 70, 75, 70, 80, 100, 90, 0, 90,100, 0, 110, 0, 110, 120, 100, 130, 130,75,140, 0, 150, 150, 0, 160, 100, 0,175, 0, 200),  "Part of Monopoly" = NA, Description = NA, "Cost to Occupy" = NA, Effect = NA, "Effect on Player_1" = NA,"Effect on Player_2" = NA,"Effect on Player_3" = NA,"Effect on Player_4" = NA, "Effect on Bank"= NA)
...
move_player <- function() {
  ...
  current_class <- current_square$Class
   ...
   if (current_class == "Railroad Tile") {#standin function to make sure things are working
    print("Railroad Tile")
     if(is.na(new_board_squares[new_location, "Owner"]) & player_stats[player_turn, "Money"] > 2 * new_board_squares[new_location, "Mortgage Price"]) {
  ...
  } else if (current_class == "Chance Tile") {
    print("Chance Tile")
    Draw_Chance_Card()
  ...
  } else if (current_class == "Go Tile") {
    print("Go Tile")
  } else if (current_class == "Luxury Tax") {
    print("Luxury Tax")
    player_stats[[player_turn, "Money"]]<<- player_stats[[player_turn, "Money"]] -75
    bank$Money <<- bank$Money + 75
  } else if (current_class == "Income Tax") {
    print("Income Tax")
    player_stats[[player_turn, "Money"]]<<- player_stats[[player_turn, "Money"]] -200
    bank$Money <<- bank$Money + 200
    
  } else if (current_class == "Go to Jail Tile") {#this won't ever happen
    print("Go to Jail Tile")
    
  } else if (current_class == "Jail") {
    print("Just Visitng")
  } else if (current_class == "Free Parking") {
    print("Free Parking")
  } else if (current_class == "Property Tile") {
    print("Property Tile")
    if(is.na(new_board_squares[new_location, "Owner"]) & player_stats[player_turn, "Money"] > 2 * new_board_squares[new_location, "Mortgage Price"]) {
  player_stats[player_turn, "Money"] <<- player_stats[player_turn, "Money"] - 2 * new_board_squares[new_location, "Mortgage Price"]
  bank$Money <<- bank$Money + 2 * new_board_squares[new_location, "Mortgage Price"]
  ...
  } else if (current_class == "Utility Tile") {
    print("Utility Tile")
...
move_player()
removed_Chance_cards
removed_community_cards
CommunityChestCards
player_stats
new_board_squares
...
```{r student4}
player<-function(){#does not take an input, uses the value of player_turn to advance the player_turn counter by 1, if the player_turn value sets x to be greater than the number of players (rows in the "player" tibble, then it reset x to be 1 and adds one to the global turn counter "total_turns"
  x <- player_turn
  if(x > nrow(player_stats)){player_turn <<-1#
  total_turns<<-total_turns + 1}#total turn counter
  player_turn<<-player_turn+1#next turn's player number
  return(player_turn-1)#current player number
  }
player()
player_turn

community_card[1, 'Drew By']
...
move<- function(x){
  ...
  if(new_location == 3 || new_location == 18 || new_location == 34){
    Draw_Community_Card()}
  if(total_sum == 0 || new_location == 31){#conditions for going to jail
...
move_player<-function(){#this function is going to get ridiculously complex 
