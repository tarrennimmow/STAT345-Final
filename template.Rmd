...
return(token_selection(c(...))$Name%>%assign_players())#janky function glueing the other two into a single command
}
...
#takes money from bank and gives it to the players
players<-player_select("?","?","?","?")#test with random names
reset_board()
players <- players %>%
  mutate(Money = NA_real_,Location = NA_character_,
         Properties = NA_character_,'Get Out of Jail Free' = NA_real_)
bank<-tibble(Money = 15140,
         houses = 32, hotels = 12)
bank$Money<- bank$Money-1500*nrow(players)
players$Money<- 1500
players$Location<-1
new_board_squares$Occupants[1] <-list(players$Name)
new_board_squares$Occupants[[1]]



#card effects( street repair,get out of jail free card, birthday card need to be worked on, get out of jail free needs to update a stat indicating that, birday card needs to remove $10 from each other player as well,


reset_CommunityChestCards<-function(){
CommunityChestCards <- data.frame(card = c("Advance to Go (Collect $200)", "Bank error in your favor. Collect $200", "Doctor’s fee. Pay $50", "From sale of stock you get $50", "Get Out of Jail Free", "Go to Jail. Go directly to jail, do not pass Go, do not collect $200", "Holiday fund matures. Receive $100", "Income tax refund. Collect $20", "It is your birthday. Collect $10 from every player", "Life insurance matures. Collect $100", "Pay hospital fees of $100", "Pay school fees of $50", "Receive $25 consultancy fee", "You are assessed for street repair. $40 per house. $115 per hotel", "You have won second prize in a beauty contest. Collect $10", "You inherit $100"))
CommunityChestCards%>% mutate(
  Money = c(200, 200, -50, 50, NA, NA, 100, 20, 10*(nrow(players)-1), 100, -100, -50, 25, NA, 10, 100),
  Property = NA,
  Location = c(1, 1, NA, NA, NA, 31, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA)) ->> CommunityChestCards
}
...
move_player <- function(x) {
  ...
  new_location <- players[[x, "Location"]] + total_sum#moves the player to the new location
  ...
  if (new_location > 40) {#checks if the player passes 40th square
    ...
    players[x, "Money"]<<-  players[x, "Money"]+ 200#adds 200 dollars to that players money
    bank$Money<<- bank$Money-200 #takes away the money from the bank
    passed_Go <- c("Player: ", players$Name[x], "passed Go")
  ...
  players[x, "Location"] <<- new_location#moves player to the correct space after go
  new_location<<-new_location
  
 return(players)#displays the new players stat
  return(new_location)
...
Draw_Community_Card<- function(){
  ...
  drawn_card<-sample(1:nrow(CommunityChestCards), 1)
  card<-(CommunityChestCards[drawn_card,])
CommunityChestCards<<-CommunityChestCards[-drawn_card,]
return(card)
}
...
run_move_player <- function() {
  ...
  return(list(players = players, location_tally = location_tally))
}
